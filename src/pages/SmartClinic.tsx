      ...
      <div className="p-4 bg-white border-t border-slate-100 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          <button 
            ...
            className="flex flex-col items-center gap-1.5 p-3 rounded-2xl bg-blue-50 hover:bg-litcBlue hover:text-white transition-all group border border-blue-100 shadow-sm"
          >
            <div className="w-8 h-8 rounded-xl bg-white flex items-center justify-center text-litcBlue group-hover:bg-white/20 group-hover:text-white transition-all">
              {loading === 'lab' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Microscope className="w-4 h-4" />}
            </div>
            <span className="text-[9px] font-black">تحليل مختبر</span>
          </button>
          <button 
            onClick={() => medInputRef.current?.click()}
            className="flex flex-col items-center gap-1.5 p-3 rounded-2xl bg-amber-50 hover:bg-litcOrange hover:text-white transition-all group border border-amber-100 shadow-sm"
          >
            <div className="w-8 h-8 rounded-xl bg-white flex items-center justify-center text-litcOrange group-hover:bg-white/20 group-hover:text-white transition-all">
              {loading === 'med' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Pill className="w-4 h-4" />}
            </div>
            <span className="text-[9px] font-black">قارئ أدوية</span>
          </button>
          <button 
            onClick={() => foodInputRef.current?.click()}
            className="flex flex-col items-center gap-1.5 p-3 rounded-2xl bg-emerald-50 hover:bg-emerald-500 hover:text-white transition-all group border border-emerald-100 shadow-sm"
          >
            <div className="w-8 h-8 rounded-xl bg-white flex items-center justify-center text-emerald-500 group-hover:bg-white/20 group-hover:text-white transition-all">
              {loading === 'food' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Utensils className="w-4 h-4" />}
            </div>
            <span className="text-[9px] font-black">محلل وجبات</span>
          </button>
        </div>
        ...
        <div className="relative flex items-center gap-2">
          <input 
            ...
            placeholder="اسأل عن سياسة التأمين أو أي استفسار طبي..."
            className="flex-1 bg-slate-50 border-2 border-slate-100 rounded-2xl py-3 pr-5 pl-12 font-bold text-xs outline-none focus:border-litcBlue focus:ring-4 focus:ring-litcBlue/5 transition-all"
          />
          <button 
            onClick={handleSendMessage}
            disabled={!input.trim() || isChatting}
            className="absolute left-2 w-10 h-10 bg-litcBlue text-white rounded-xl flex items-center justify-center hover:bg-litcDark transition-all disabled:opacity-50 shadow-lg shadow-litcBlue/20"
          >
