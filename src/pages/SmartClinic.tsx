  ...
  return (
    <div className="flex flex-col h-full font-cairo bg-slate-50/30" dir="rtl">
      ...
      <div className="p-4 bg-white border-t border-slate-100 space-y-4">
        ...
        {/* Micro-Buttons (Pill shape) */}
        <div className="flex flex-wrap justify-center gap-2">
          <button 
            onClick={() => medInputRef.current?.click()}
            className="flex items-center gap-2 px-4 py-2 rounded-full bg-amber-50 text-amber-700 border border-amber-100 hover:bg-amber-100 transition-all text-[10px] font-black"
          >
            {loading === 'med' ? <Loader2 className="w-3 h-3 animate-spin" /> : <Pill className="w-3 h-3" />}
            فحص دواء
          </button>
          <button 
            onClick={() => labInputRef.current?.click()}
            className="flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50 text-blue-700 border border-blue-100 hover:bg-blue-100 transition-all text-[10px] font-black"
          >
            {loading === 'lab' ? <Loader2 className="w-3 h-3 animate-spin" /> : <Microscope className="w-3 h-3" />}
            لائحة طبية
          </button>
          <button 
            onClick={() => foodInputRef.current?.click()}
            className="flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100 hover:bg-emerald-100 transition-all text-[10px] font-black"
          >
            {loading === 'food' ? <Loader2 className="w-3 h-3 animate-spin" /> : <Utensils className="w-3 h-3" />}
            مساعد إداري
          </button>
        </div>
